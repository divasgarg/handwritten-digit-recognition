...
st.write('Upload a 28x28 grayscale image of a handwritten digit, or any small digit image (it will be resized).')
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.error('Could not load CNN model. Train the model first by running `python -m src.train_cnn`.')
        st.text(str(e))
    else:
